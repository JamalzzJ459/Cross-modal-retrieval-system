# Copyright (c) Facebook, Inc. and its affiliates.
# All rights reserved.
#
# This source code is licensed under the license found in the
# LICENSE file in the root directory of this source tree.
#
from logging import getLogger

from PIL import ImageFilter, Image
import numpy as np
import torchvision.datasets as datasets
import torchvision.transforms as transforms
import torch.utils.data as data
import torch
import scipy.io as sio
import os
import h5py
import random
from torchvision.transforms import Compose, Resize, CenterCrop, ToTensor, Normalize
from torchvision import transforms, datasets
from src.simple_tokenizer import SimpleTokenizer as Tokenizer
logger = getLogger()
#python UCCH.py --data_name mirflickr25k_fea --bit 128 --alpha 0.7 --num_hiden_layers 3 2 --margin 0.2 --max_epochs 20 --train_batch_size 256 --shift 0.1 --lr 0.0001 --optimizer Adam
class Sampler():
    def __init__(self, root, paths):
        self.root = root
        if isinstance(paths, np.ndarray):
            if len(paths.shape) == 1 or paths.shape[0] == 1 or paths.shape[1] == 1:
                paths = paths.reshape([-1]).tolist()
        self.paths = paths

    def __getitem__(self, item):
        path = self.paths[item]
        if isinstance(path, np.ndarray):
            if len(path.shape) >= 2:
                return Image.fromarray(path, mode='RGB')
            else:
                path = path[0]
        return Image.open(os.path.join(self.root, path))

    def __len__(self):
        return len(self.paths)

def text_transform(text):
    return text

class CMDataset(data.Dataset):
    """
    Load precomputed captions and image features
    Possible options: f30k_precomp, coco_precomp
    """

    def __init__(
        self,
        data_name,
        return_index=False,
        partition='train',
        imageResolution=224,
        is_train = True,
        two_crop=False
    ):
        self.data_name = data_name
        self.partition = partition
        self.if_tea = False
        self.two_crop = two_crop
        training = 'train' in partition.lower()
        mean = [0.485, 0.456, 0.406]
        std = [0.229, 0.224, 0.225]
        normalize = transforms.Normalize(mean=mean, std=std)
        self.transform = Compose([
         transforms.RandomResizedCrop(224, scale=(0.2, 1.)),
         transforms.RandomHorizontalFlip(),
         transforms.ToTensor(),
         normalize,
         ])
        self.return_index = return_index
        self.if_open_file = False
        self.open_data()
        self.tokenizer = Tokenizer()
        self.SPECIAL_TOKEN = {"CLS_TOKEN": "<|startoftext|>", "SEP_TOKEN": "<|endoftext|>",
                              "MASK_TOKEN": "[MASK]", "UNK_TOKEN": "[UNK]", "PAD_TOKEN": "[PAD]"}
        self.maxWords = 32
    def open_data(self):
        # mirflickr25k mirflickr25k_fea MSCOCO_fea nus_wide_tc10_fea IAPR-TC12_fea
        if self.data_name.lower() == 'mirflickr25k':
            data = MIRFlickr25K(self.partition)
        elif self.data_name.lower() == 'nus_wide_tc10':
            data = NUSWIDE(self.partition)
        elif self.data_name.lower() == 'mirflickr25k_fea':
            data = MIRFlickr25K_fea(self.partition)
        elif self.data_name.lower() == 'mirflickr25k_fea_tea':
            self.if_open_file = True
            data = MIRFlickr25K_fea_tea(self.partition)
        elif self.data_name.lower() == 'iapr_fea':
            data = IAPR_fea(self.partition)
        elif self.data_name.lower() == 'nuswide_fea':
            self.if_open_file = True
            data = NUSWIDE_fea(self.partition)
        elif self.data_name.lower() == 'mscoco_fea':
            data = MSCOCO_fea(self.partition)
        elif self.data_name.lower() == 'mscoco_new_fea':
            self.if_open_file = True
            data = MSCOCO_new_fea(self.partition)

        if len(data) == 3:
            (self.imgs, self.texts, self.labels) = data
            self.imgs = self.imgs
        elif len(data) == 5:
            (self.imgs, self.texts,self.img_tea,self.txt_tea,self.labels) = data
            self.imgs = self.imgs
            self.if_tea = True
        else:
            (self.imgs, self.texts, self.labels, root) = data
            self.imgs = Sampler(root, self.imgs)
        self.length = self.labels.shape[0]
    def _load_image(self, index: int) -> torch.Tensor:
        if(self.if_open_file):
            image_path = self.imgs[index][0].strip()
            #image_path = self.imgs[index].strip()
            # print(image_path)
            image = Image.open(image_path).convert("RGB")
            if(self.two_crop):
                image1 = self.transform(image)
                image2 = self.transform(image)
                image = torch.cat([image1, image2], dim=0)
            else:
                image = self.transform(image)
        else:
            image = self.imgs[index]

        return image
    def _load_text(self, index: int) -> torch.Tensor:
        text = self.texts[index]
        use_cap = text[random.randint(0, len(text) - 1)]

        words = self.tokenizer.tokenize(use_cap)
        words = [self.SPECIAL_TOKEN["CLS_TOKEN"]] + words
        total_length_with_CLS = self.maxWords - 1
        if len(words) > total_length_with_CLS:
            words = words[:total_length_with_CLS]
        
        words = words + [self.SPECIAL_TOKEN["SEP_TOKEN"]]
        text = self.tokenizer.convert_tokens_to_ids(words)

        while len(text) < self.maxWords:
            text.append(0)
        text = torch.tensor(text)

        return text
    def __getitem__(self, index):
        image = self._load_image(index)
        #text = self._load_text(index)
        text = self.texts[index]
        if isinstance(self.imgs, Sampler):
            multi_crops = list(map(lambda trans: trans(image), self.trans))
            text = list(map(lambda trans: trans(text), [text_transform] * len(self.trans)))
        else:
            multi_crops = image
            text = text

        label = self.labels[index]
        if (self.if_tea):
            return index,multi_crops,text,self.img_tea[index],self.txt_tea[index],label
        if self.return_index:
            return index, multi_crops, text, label
        return multi_crops, text, label
        # return multi_crops, text, index

    def __len__(self):
        return self.length

def MIRFlickr25K(partition):
    import h5py
    imgs = h5py.File('./data/MIRFLICKR25K/mirflickr25k-iall.mat', mode='r')['IAll'][()]
    root = './data/MIRFLICKR25K/'
    tags = sio.loadmat('./data/MIRFLICKR25K/mirflickr25k-yall.mat')['YAll']
    labels = sio.loadmat('./data/MIRFLICKR25K/mirflickr25k-lall.mat')['LAll']

    inx = np.arange(imgs.shape[0])
    np.random.shuffle(inx)
    imgs, tags, labels = imgs[inx], tags[inx], labels[inx]
    test_size = 2000
    eval_size = 2000
    if 'test' in partition.lower():
        imgs, tags, labels = imgs[-test_size::], tags[-test_size::], labels[-test_size::]
    if 'eval' in partition.lower():
        imgs, tags, labels = imgs[-test_size-eval_size:-test_size], tags[-test_size-eval_size:-test_size], labels[-test_size-eval_size:-test_size]
    elif 'train' in partition.lower():
        imgs, tags, labels = imgs[0: -test_size], tags[0: -test_size], labels[0: -test_size]
        all_indices = list(range(imgs.shape[0]))
        samples_per_epoch = 5000
        samples = random.sample(all_indices, samples_per_epoch)
        imgs, tags, labels = imgs[samples], tags[samples], labels[samples]
    else:
        imgs, tags, labels = imgs[0: -test_size], tags[0: -test_size], labels[0: -test_size]

    return imgs.transpose([0, 3, 2, 1]), tags, labels, root


def NUSWIDE(partition):
    imgs = h5py.File('./data/NUS-WIDE-TC10/nus-wide-tc10-iall.mat')['IAll'][()]
    root = './data/NUS-WIDE-TC10/'
    tags = sio.loadmat('./data/NUS-WIDE-TC10/nus-wide-tc10-yall.mat')['YAll']
    labels = sio.loadmat('./data/NUS-WIDE-TC10/nus-wide-tc10-lall.mat')['LAll']
    
    inx = np.arange(imgs.shape[0])
    np.random.shuffle(inx)
    imgs, tags, labels = imgs[inx], tags[inx], labels[inx]
    test_size = 2100
    if 'test' in partition.lower():
        imgs, tags, labels = imgs[-test_size::], tags[-test_size::], labels[-test_size::]
    else:
        imgs, tags, labels = imgs[0: -test_size], tags[0: -test_size], labels[0: -test_size]

    return imgs.transpose([0, 3, 2, 1]), tags, labels, root

def MIRFlickr25K_fea(partition):
    root = './data/MIRFLICKR25K/'
    data_imgs = sio.loadmat(os.path.join(root, 'feature_map.mat'))['image_stu_fea']
    data_txts = sio.loadmat(os.path.join(root, 'feature_map.mat'))['text_stu_fea']
    
    data_imgs_tea = sio.loadmat(os.path.join(root, 'feature_map.mat'))['image_fea']
    data_txts_tea = sio.loadmat(os.path.join(root, 'feature_map.mat'))['text_fea']
    labels = sio.loadmat(os.path.join(root, 'feature_map.mat'))['label']
    test_size = 2000
    if 'test' in partition.lower():
        data_img, data_txt, label = data_imgs[-test_size::], data_txts[-test_size::], labels[-test_size::]
        data_img_tea, data_txt_tea = data_imgs_tea[-test_size::], data_txts_tea[-test_size::]
        
    elif 'train' in partition.lower():
        data_img, data_txt, label = data_imgs[0: -test_size], data_txts[0: -test_size], labels[0: -test_size]
        data_img_tea, data_txt_tea = data_imgs_tea[0: -test_size], data_txts_tea[0: -test_size]
        #all_indices = list(range(data_imgs.shape[0]))
        #samples_per_epoch = 5000
        #samples = random.sample(all_indices, samples_per_epoch)
        #data_img, data_txt, label = data_imgs[samples], data_txts[samples], labels[samples]
    else:
        data_img, data_txt, label = data_imgs[0: -test_size], data_txts[0: -test_size], labels[0: -test_size]
        data_img_tea, data_txt_tea = data_imgs_tea[0: -test_size], data_txts_tea[0: -test_size]
    return data_img, data_txt, data_img_tea,data_txt_tea,label
def MIRFlickr25K_fea_tea(partition):
    root = './data/MIRFLICKR25K/'
    data_imgs = sio.loadmat(os.path.join(root, 'feature_map.mat'))['image_path']
    data_txts = sio.loadmat(os.path.join(root, 'feature_map.mat'))['text_fea']
    labels = sio.loadmat(os.path.join(root, 'feature_map.mat'))['label']
    
    #data_txts = sio.loadmat(os.path.join(root, 'caption.mat'))['caption']
    #data_imgs = sio.loadmat(os.path.join(root, 'index.mat'))['index']
    #labels = sio.loadmat(os.path.join(root, 'label.mat'))['category']
        
    test_size = 2000
    if 'test' in partition.lower():
        data_img, data_txt, label = data_imgs[-test_size::], data_txts[-test_size::], labels[-test_size::]
    elif 'train' in partition.lower():
        data_img, data_txt, label = data_imgs[0: 5000], data_txts[0: 5000], labels[0: 5000]
        #all_indices = list(range(data_imgs.shape[0]))
        #samples_per_epoch = 5000
        #samples = random.sample(all_indices, samples_per_epoch)
        #data_img, data_txt, label = data_imgs[samples], data_txts[samples], labels[samples]
    else:
        data_img, data_txt, label = data_imgs[0: -test_size], data_txts[0: -test_size], labels[0: -test_size]

    return data_img, data_txt, label

def IAPR_fea(partition):
    root = './data/IAPR-TC12/'
    file_path = os.path.join(root, 'iapr-tc12-rand.mat')
    data = sio.loadmat(file_path)

    valid_img = data['VDatabase'].astype('float32')
    valid_txt = data['YDatabase'].astype('float32')
    valid_labels = data['databaseL']

    test_img = data['VTest'].astype('float32')
    test_txt = data['YTest'].astype('float32')
    test_labels = data['testL']

    data_img, data_txt, labels = np.concatenate([valid_img, test_img]), np.concatenate([valid_txt, test_txt]), np.concatenate([valid_labels, test_labels])

    test_size = 2000
    if 'test' in partition.lower():
        data_img, data_txt, labels = data_img[-test_size::], data_txt[-test_size::], labels[-test_size::]
    else:
        data_img, data_txt, labels = data_img[0: -test_size], data_txt[0: -test_size], labels[0: -test_size]

    return data_img, data_txt, labels


def NUSWIDE_fea(partition):
    root = './data/nuswide/'
    test_size = 2100
    data_img = sio.loadmat(root + 'index.mat')['index']
    txt_path = root + 'caption.txt'
    labels = sio.loadmat(root + 'label.mat')['category']
    
    with open(txt_path, "r") as f:
        captions = f.readlines()
    data_txt = np.asarray([[item.strip()] for item in captions])
    print(data_txt[:3])
    test_size = 2100
    if 'test' in partition.lower():
        data_img, data_txt, labels = data_img[-test_size::], data_txt[-test_size::], labels[-test_size::]
    elif 'train' in partition.lower():
        data_img, data_txt, labels = data_img[0: 10000], data_txt[0: 10000], labels[0: 10000]
        #data_img, data_txt, labels = data_img[0: -test_size], data_txt[0: -test_size], labels[0: -test_size]
    else:
        data_img, data_txt, labels = data_img[0: -test_size], data_txt[0: -test_size], labels[0: -test_size]
    return data_img, data_txt, labels

def MSCOCO_fea(partition):
    root = './data/MSCOCO/'
    path = root + 'MSCOCO_deep_doc2vec_data_rand.h5py'
    data = h5py.File(path)
    data_img = data['XAll'][()]
    data_txt = data['YAll'][()]
    labels = data['LAll'][()]


    test_size = 5000
    if 'test' in partition.lower():
        data_img, data_txt, labels = data_img[-test_size::], data_txt[-test_size::], labels[-test_size::]
    
    elif 'train' in partition.lower():
        data_img, data_txt, labels = data_img[0: 10000], data_txt[0: 10000], labels[0: 10000]
    else:
        data_img, data_txt, labels = data_img[0: -test_size], data_txt[0: -test_size], labels[0: -test_size]

    return data_img, data_txt, labels

def MSCOCO_new_fea(partition):
    root = './data/coco_fea/'
    data_img = sio.loadmat(os.path.join(root, 'feature_map.mat'))['image']
    data_txt = sio.loadmat(os.path.join(root, 'feature_map.mat'))['text_fea']
    labels = sio.loadmat(os.path.join(root, 'feature_map.mat'))['label']
    #data_txt = sio.loadmat(os.path.join(root, 'caption.mat'))['caption'].squeeze(0)
    #data_img = sio.loadmat(os.path.join(root, 'index.mat'))['index']
    #labels = sio.loadmat(os.path.join(root, 'label.mat'))['category']
    test_size = 5000
    if 'test' in partition.lower():
        data_img, data_txt, labels = data_img[-test_size::], data_txt[-test_size::], labels[-test_size::]
    elif 'extract' in partition.lower():
        data_img, data_txt, labels = data_img[0: 10000], data_txt[0: 10000], labels[0: 10000]
        #data_img, data_txt, labels = data_img[0: -test_size], data_txt[0: -test_size], labels[0: -test_size]
    elif 'train' in partition.lower():
        data_img, data_txt, labels = data_img, data_txt, labels

    else:
        data_img, data_txt, labels = data_img[0: -test_size], data_txt[0: -test_size], labels[0: -test_size]
    return data_img, data_txt, labels