...
from NCE.NCEAverage import new_contra,NCEAverage
...
def main():
    ...
    def train(epoch):
        ...
        for batch_idx, (idx, images, texts, _) in enumerate(train_loader):
            ...
            images_outputs2 = [image_model(im) for im in images]
            texts_outputs = [text_model(txt.float()) for txt in texts]
            texts_outputs2 = [text_model(txt.float()) for txt in texts]
            #计算C(ft,ft)和(fi,fi)
            ...
            Lc = l_loss + ab_loss
            #Lr = criterion.get_it_loss(torch.cat(images_outputs), torch.cat(texts_outputs),C)
            Lr = criterion(torch.cat(images_outputs), torch.cat(texts_outputs),torch.cat(images_outputs2), torch.cat(texts_outputs2))
            #Lc2 = newcon(torch.cat(images_outputs), torch.cat(texts_outputs),torch.cat(images_outputs2), torch.cat(texts_outputs2),C)
            #loss = (Lc+ Lc2)* args.alpha + Lr * (1. - args.alpha)
            #loss = (args.alpha *Lc + (1.0 - args.alpha) * Lc2)* args.beta + Lr* (1.0 - args.beta)
